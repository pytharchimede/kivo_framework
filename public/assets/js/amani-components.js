(() => {
  const initSelectSearch = (select) => {
    if (select.dataset.amaniEnhanced === "1") return;
    select.dataset.amaniEnhanced = "1";
    select.classList.add("amani-native-select");
    ...
    wrapper.className = "amani-select-search";

    const control = document.createElement("div");
    control.className = "amani-select-control";
    control.tabIndex = 0;

    const input = document.createElement("input");
    input.type = "search";
    input.className = "amani-select-input";
    input.placeholder = placeholder;
    input.autocomplete = "off";

    const menu = document.createElement("div");
    menu.className = "amani-select-menu";
    ...
    const renderBadges = () => {
      control.querySelectorAll(".amani-select-badge").forEach((badge) => badge.remove());
      if (!multiple) return;
      selectedValues().forEach((value) => {
        const item = source.find((option) => option.value === value);
        const badge = document.createElement("span");
        badge.className = "amani-select-badge";
    ...
    const renderMenu = () => {
      ...
      if (!items.length) {
        ...
        empty.className = "amani-select-empty";
      ...
      items.forEach((option) => {
        ...
        button.type = "button";
        button.className = "amani-select-option";
  ...
  const previewFile = (input) => {
    const zone = input.closest("[data-amani-dropzone], [data-finea-dropzone]");
    const preview = zone?.querySelector("[data-amani-file-preview], [data-finea-file-preview]");
  ...
  const initDropzone = (zone) => {
    if (zone.dataset.amaniDropzoneEnhanced === "1") return;
    zone.dataset.amaniDropzoneEnhanced = "1";
  ...
  const init = () => {
    document.querySelectorAll("select[data-amani-select-search], select[data-finea-select-search], select[data-select-search]").forEach(initSelectSearch);
    document.querySelectorAll("[data-amani-dropzone], [data-finea-dropzone], [data-dropzone]").forEach(initDropzone);
  };

  document.addEventListener("DOMContentLoaded", init);
  window.AmaniComponents = { init };
})();
